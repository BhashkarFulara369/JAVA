...
<div class="container mt-5">
    <div class="text-center">
        <h2 class="fw-bold"><i class="fas fa-bus"></i> Bus Ticket Booking</h2>
        <p>Book your bus tickets easily with our seamless booking system.</p>
    </div>
    ...
    <form id="bookingForm">
        <div class="row mt-4">
            ...
            <div class="col-md-4">
                <div class="booking-card">
                    <h3 class="section-title"><i class="fas fa-chair"></i> Seat Selection</h3>

                    <div class="d-flex align-items-center mb-3">
                        <button type ="button" id="showSeatsBtn" class="btn btn-primary">Show Available Seats</button>
                    ...
                    <div id="seatOverlay" class="overlay">
                        <div class="seat-container">
                            ...
                            <button type = "button" id="confirmSeat" class="btn btn-success mt-3" disabled>Confirm Selection</button>
                        </div>
                    </div>

                    <h3 class="section-title mt-4"><i class="fas fa-credit-card"></i> Payment</h3>
                   <button type="button" class="btn btn-primary w-100 mt-3" onclick="redirectToPayment()">Book Now</button>
                </div>
            </div>
        </div>
    </form>
...
function redirectToPayment() {
    ...
    const seatNumber = document.getElementById("selectedSeat").value; // Ensure correct ID

    if (!seatNumber) {
        alert("Please select a seat before booking.");
        return; // Prevent redirect if no seat is chosen
    }

    const url = `payment_page.jsp?username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&source=${encodeURIComponent(source)}&destination=${encodeURIComponent(destination)}&travelDate=${encodeURIComponent(travelDate)}&busType=${encodeURIComponent(busType)}&seatNumber=${encodeURIComponent(seatNumber)}`;
    window.location.href = url;
     ...
     // Generate seat numbers from 1 to 40
     const busSeats = Array.from({ length: 40 }, (_, i) => (i + 1).toString());
     ...
     showSeatsBtn.addEventListener("click", function () {
         ...
         busSeats.forEach(seat => {
             const isBooked = Math.random() < 0.3; // Randomly mark some seats as unavailable
             ...
             if (!isBooked) {
                 seatElement.addEventListener("click", function () {
                     ...
                     selectedSeatDisplay.innerText = "Seat: " + this.innerText;
